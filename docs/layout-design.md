...
终端区域支持独立配色方案，内置 16 个预设（CC Desk 浅/暗 + Dracula / Gruvbox / Nord 等程序员主题）。由 `src/config/terminalThemes.ts` 定义；`appStore.terminalTheme` 驱动 xterm 字符栅格；`computeTerminalSurfaceVars` 产出局部 CSS 变量（`--terminal-surface-bg`/`--terminal-scrollbar`）驱动终端容器背景、空态、滚动条。切换 GUI 浅/暗不影响终端。仅保证 16 色 ANSI 一致，256 色沿用 xterm 默认，truecolor 不受影响。
