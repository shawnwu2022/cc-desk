...
## Review-driven epoch admission correction

- b752df4a, CI #176 (35827019832), Windows job 107070888638: all 497 library and 16 other active Rust tests passed, including the 15 document cases; 15 library and 3 integration cases remain ignored. Native Tauri adapter compiled. Frontend job succeeded. Rust job failed only for one document.rs line layout and one needless borrow in initialization_script; both complete diagnostics were read.
- Author self-review identified that a Ready document could remain locally Ready after direct registry revocation/replacement before its page callback arrives. Existing coordinator/status checks already prevented protected launch effects/status reads, but admission could still parse the stale caller's JSON first. This was not a demonstrated unauthorized process launch.
- Test-only be24a28b adds four edge cases. CI #177 (35827344690), Windows job 107071882586: 500 library passed, exactly one failed, 15 ignored. EpochInvalidationPrecedesTypedDecode_16 returned INVALID_REQUEST instead of FORBIDDEN. The first-navigation, inclusive query boundary and cross-backend-proof cases already passed; they are not claimed as additional RED evidence. Frontend succeeded. The two known style/lint issues plus one edge-test layout also remained.
- Correction adds a side-effect-free RunRegistry::check_caller using the existing authority predicate. Both internal and native admission check current backend instance/epoch before typed decoding. The native path first obtains its witness and releases the ResourceTable guard, then checks the registry; neither lock is held for deserialization. The coordinator and status read retain their original revalidation because a pre-decode check does not reserve authority against later revocation.
- All 19 Rust document assertions and 5 JS bootstrap assertions are retained. Only the exact observed formatting/needless-borrow corrections accompany this fix. No dependency, registry state machine, active startup or IPC registration change is included. Final GREEN still requires a fresh complete CI run on the successor commit; record that exact result in PR metadata without creating an untested documentation successor.
