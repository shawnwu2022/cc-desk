...
已存档会话可永久删除（单删 + 跨项目批量），删除即删会话文件 + 清 `archivedSessions` 标记，不可恢复。运行中（claimed）会话禁止删除。

### Delete_RunningTabDisabled_001 — 运行中 tab 拒删

**目标**：已存档区某会话若正在运行（有对应 running tab），删除按钮置灰、点击无响应、tooltip 提示「运行中」

**前置条件**：某存档会话正在运行（已在终端打开该会话的 running tab，且该 tab 的 sessionId 已匹配到已存档 id）

**操作步骤**：
1. 进入管理页（Home），滚动到底部「已存档会话」区块
2. 找到正在运行的那个会话，观察其右侧删除按钮
3. 鼠标悬停查看 tooltip
4. 尝试点击删除按钮

**预期结果**：
- 步骤 2：删除按钮置灰（`disabled`，`opacity` 变淡）
- 步骤 3：tooltip 显示「会话运行中，请先关闭标签页」（runningSessionHint）
- 步骤 4：点击无响应（不弹确认框、标记不变、文件不删）
- 全局树 ProjectNode 内联面板的已存档弹层中，该会话删除按钮同样置灰

### Delete_RealDeleteClaudeResumeGone_002 — 真实删除后 CLI 不再列出

**目标**：删除某个已存档会话后，终端 `claude --resume` 列表不再包含该会话，且该会话 JSONL 文件已消失

**前置条件**：有一个已存档会话（可先在历史树上存档产生），记录其 sessionId 与项目路径

**操作步骤**：
1. 进入管理页「已存档会话」区块，点击该会话的「删除会话」按钮
2. 在弹出的确认框点击确定
3. 在一个新终端执行 `claude --resume`，查看会话列表
4. 在文件系统定位该项目的 `~/.claude/projects/<encoded-path>/` 目录，检查对应文件

**预期结果**：
- 步骤 2 后：该会话从已存档列表消失，`~/.cc-box/projects.json` 的 `archivedSessions` 对应标记清除
- 步骤 3：`claude --resume` 交互列表不含该 sessionId
- 步骤 4：`<sessionId>.jsonl`（及 `.txt`）文件不存在（已物理删除）

### Delete_CrossProjectBatch_001 — 跨项目批量全流程

**目标**：已存档区进批量模式，勾选跨项目多条会话，确认后批量删除，标记与列表正确收敛

**前置条件**：两个不同项目各至少有一个已存档会话；无运行中会话

**操作步骤**：
1. 进入管理页「已存档会话」区块，点击「批量删除」
2. 勾选项目 A 的一条会话 + 项目 B 的一条会话（跨项目）
3. 观察底部确认条（含已选计数）
4. 点击「确定删除选中的 N 个会话」
5. 观察列表刷新与 `~/.cc-box/projects.json`

**预期结果**：
- 步骤 1：出现「取消批量」按钮，每条会话前出现勾选框，恢复按钮禁用
- 步骤 2：两项目会话均可勾选（跨项目无冲突）
- 步骤 3：确认条显示选中计数 N=2
- 步骤 4-5：勾选的两条从已存档区消失，列表刷新；`archivedSessions` 中项目 A、B 对应标记均清除；其余未勾选会话保留
- 批量模式下勾选框对运行中会话 `disabled`

### Delete_PartialFailureRetry_001 — 部分失败提示 + 重试收敛

**目标**：删除一批含一个只读目录下的会话时，删除失败提示；整批标记保留（任一失败整体不写 projects.json）；重试后已删文件跳过、剩余删除成功，标记清除收敛

**前置条件**：准备两个已存档会话 S1（正常目录）、S2（所在项目目录设为只读 / 无删除权限，模拟删除失败）

**操作步骤**：
1. 进入管理页「已存档会话」区块，进批量模式勾选 S1、S2
2. 点击确认删除
3. 观察失败提示
4. 检查 `~/.cc-box/projects.json`
5. 将 S2 所在目录恢复读写权限
6. 再次勾选 S1、S2 点击确认删除

**预期结果**：
- 步骤 2-3：弹窗提示「删除失败」（deleteFailed）；S1、S2 都仍在已存档列表（未消失）
- 步骤 4：`archivedSessions` 中 S1、S2 标记均保留（任一失败整体不写 projects.json）
- 步骤 6 后：删除成功，S1、S2 从列表消失，标记清除收敛（S1 文件已删，重试时「文件不存在视为已删」跳过；S2 本次真删除）

### Delete_WindowsCaseVariant_001 — Windows 大小写路径变体验证兜底修复

**目标**：归档某 Windows 路径项目后，全局已存档区能正常显示（验证规范化 key 兜底：存档用 `E:\Repo`、显示/删除走 `e:/repo` 等价 key 也能命中），删除成功

**前置条件**：Windows 系统；某项目以 `E:\Repo` 形式归档过（`archivedSessions` 的 key 为 `E:\Repo`），而当前会话以 `e:/repo` 形式加载（大小写/斜杠变体不同）

**操作步骤**：
1. 进入管理页滚动到底部「已存档会话」区块
2. 确认该 Windows 路径项目及会话被列出（name 可读）
3. 点击该会话的「删除会话」并确认
4. 检查 `~/.cc-box/projects.json`

**预期结果**：
- 步骤 2：该项目会话在全局已存档区正常显示（本来是 `E:\Repo` key，前端用 `e:/repo` 查找也能命中 —— 规范化 key 合并查找兜底生效）
- 步骤 3-4：删除成功，会话从列表消失；`archivedSessions` 中等价 key 的对应标记清除（无论原 key 大小写/斜杠形式）
...
**目标**：验证剪贴板只有截图时 Ctrl+V 触发图片分流

**前置条件**：Windows，终端内有运行中的 Claude Code 会话

**操作步骤**：
1. Win+Shift+S 截取任意区域
2. 聚焦终端按 Ctrl+V
3. 另测按 Alt+V

**预期结果**：
- 两种按键均在 CLI 输入区插入 [Image #N] 芯片，发送后模型能描述截图内容
...
**操作步骤**：
1. cmd /c "type nul | clip" 清空剪贴板（复制空格等空白文本不是空剪贴板，会走文本分支，不能替代）
2. Ctrl+V

**预期结果**：
- 无异常、无文本被贴，CLI 无图可贴维持原状（提示或无反应均可）

### 图片粘贴-多来源剪贴板

**目标**：不同截图/复制来源的格式差异由 CLI 消化

**前置条件**：同上

**操作步骤**：
1. 分别用微信截图、QQ 截图、浏览器右键复制图片后按 Ctrl+V

**预期结果**：
- 各来源均插入 [Image #N]（个别来源失败属 CLI 剪贴板格式支持范围，不崩溃即可）

### 图片粘贴-剪贴板竞态

**目标**：TOCTOU 竞态下不崩溃（best effort 语义）

**前置条件**：同上

**操作步骤**：
1. Win+Shift+S 截图
2. 按 Ctrl+V 后立刻复制其他内容

**预期结果**：
- 不崩溃；CLI 粘贴到哪份内容不做强断言（GUI 判定与 CLI 读取不保证同一快照）
