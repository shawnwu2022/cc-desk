...
针对 Codex 开发的多终端管理器，用 Tauri 2 + Vue 3 + xterm.js + portable-pty 直连 Codex CLI，在原生终端体验基础上增加外围功能和信息呈现。

## 核心思想

**面向 Codex 重度用户的多会话管理器。GUI 做增强不做替代，让 CLI 做不好的事变得容易。**

### 产品定位

- **面向谁**：已熟练使用 Codex CLI 的开发者，尤其是需要同时管理多个会话、多个项目的重度用户
- **解决什么问题**：CLI 在单会话交互上已经足够好，但在多会话并行、信息总览、跨会话状态追踪上力不从心
- **核心价值**：
  1. **多会话并行管理** — 一个窗口内同时运行多个 Codex 会话，快速切换、互不干扰
  2. **信息可视化增强** — MCP 工具详情等 CLI 不方便展示的信息，通过侧边栏面板呈现
  3. **工作流加速** — 快捷命令、prompt 片段、项目预设等 CLI 之外的外围辅助

### 设计原则

- **CLI 优先，GUI 增强** — 直接运行 Codex CLI 二进制文件，输入行为与原生终端完全一致；GUI 只做 CLI 做不好或做起来不方便的事
...
- **可逆性** — 用户可随时回到纯 CLI，GUI 不修改任何 Codex 原生配置文件，不留副作用
- **最小依赖** — 完全兼容 Codex 任何更新，无需适配 SDK API；新功能通过读取原生配置文件和 CLI 命令获取
...
- 不做 Codex 配置的编辑器（只做只读展示，编辑由 CLI/settings.json 完成）
...
```
xterm.js ←→ Tauri invoke/listen ←→ pty.rs (Rust) ←→ portable-pty ←→ Codex CLI
```

- 用户输入 → `onData` → `invoke('pty_input')` → PTY writer → Codex CLI
- CLI 输出 → PTY reader → `decode_output()`（UTF-8 优先，失败回退 GBK，兼容 Windows 中文子进程）→ `emit('pty-output')` → `term.write()` → xterm.js

### Hook 监控数据流

```
Codex CLI hook 触发 → report-hook.sh → curl POST → hook_server.rs (axum) → emit('hook-event') → stores/hook.ts (事件总线) → useStatusMonitor (状态监控)
```
...
- Sessions 面板从「当前项目扁平列表」升级为「项目→会话全局树」：终端视图内跨项目一步切换 + 并行项目状态徽标（`●N` 运行 / 琥珀点 pending）一眼可见，后端仅增 `displayNames` 字段 + 原子写（零新增 command）
...
- **多实例 stale-write 限制**：opLock 仅单 Pinia store（单实例）闭环；多实例并发改 alias/pin/archive/displayName 可能丢一项，建议单实例操作（已知限制，非 bug）
...
外部参考：[Codex 线上文档](https://code.Codex.com/docs/llms.txt)
