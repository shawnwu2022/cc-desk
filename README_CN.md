...
为每个项目设置启动选项，如 `--continue`、`--model` 或自定义参数。无需每次输入相同参数，一键以预设配置启动会话。
...
Claude Code 已无法通过 npm 安装。请使用以下方式：
...
- **Windows**: MinGW-w64 (`C:\ProgramData\mingw64\mingw64\bin` 在 PATH 中)
...
- **隐私政策**: 本程序不会向其他网络系统传输任何信息，除非用户或安装/操作者明确请求。
