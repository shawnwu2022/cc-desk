...
use crate::cli::document::{DocumentAuthority, DocumentBinding, DocumentWitness, NativeContext, DOCUMENT_HEADER, MAX_LAUNCH_WIRE_BYTES, MAX_QUERY_WIRE_BYTES};
...
impl Harness {
    fn new(ready: bool) -> Self {
        ...
        Self { f, authority, binding, table, url, headers }
    }

    fn context(&self) -> NativeContext<'_> {
        NativeContext { window_label: "main", webview_label: "main", url: &self.url }
    }

    fn query(&self) -> Result<LaunchStatus, SafeError> {
        self.binding.query_status(&self.table, &self.context(), &self.headers, &InvokeBody::Raw(br#"{"requestId":"owned-request"}"#.to_vec()))
    }

    fn start(&self) -> LaunchStatus {
        let body = InvokeBody::Raw(serde_json::to_vec(&self.f.request).unwrap());
        let (caller, request) = self.binding.start_request(&self.table, &self.context(), &self.headers, &body).unwrap();
        self.f.driver.start_routed(&caller, &request, || self.f.freeze(), |_| Ok(self.f.lease()), |_| Ok(42)).unwrap()
    }
}
...
fn D11_Document_InjectedLabelsAndExactUrlRequired_02() {
    ...
    for context in [
        NativeContext { window_label: "other", ..h.context() },
        NativeContext { webview_label: "other", ..h.context() },
        NativeContext { url: &foreign, ..h.context() },
        NativeContext { url: &remote, ..h.context() },
    ] {
        assert_eq!(h.binding.admit(&h.table, &context, &h.headers).unwrap_err().code, "FORBIDDEN");
    }
    assert_eq!(h.binding.admit(&h.table, &h.context(), &h.headers).unwrap(), h.f.caller);
}
...
fn D11_Document_ProofMissingWrongDuplicateAndOversizedRejected_03() {
    ...
    wrong.insert(DOCUMENT_HEADER, HeaderValue::from_static("00000000000000000000000000000000"));
    ...
    for headers in [HeaderMap::new(), wrong, duplicate, oversized] {
        assert_eq!(h.binding.admit(&h.table, &h.context(), &headers).unwrap_err().code, "FORBIDDEN");
    }
...
fn D11_Document_RemoteBootstrapCannotRotateRegistry_04() {
    let h = Harness::new(true);
    for url in ["https://example.invalid/", "file:///private/index.html", "http://tauri.localhost.evil.invalid/", "http://user@tauri.localhost/"] {
        assert!(DocumentAuthority::new(h.f.driver.registry().clone(), url.parse().unwrap()).is_err());
    }
    assert_eq!(h.f.driver.registry().status(&h.f.caller, "owned-request").unwrap_err().code, "LAUNCH_NOT_FOUND");
}
...
fn D11_Document_ReloadAndLateFinishNeverReauthorize_05() {
    ...
    h.authority.finished(&h.url);
    assert_eq!(h.query().unwrap_err().code, "FORBIDDEN");
    assert_eq!(h.f.driver.registry().resource(&h.f.caller, &status.run).unwrap_err().code, "FORBIDDEN");
...
fn D11_Document_NativeResourceIdentityNotLabelOrId_07() {
    ...
    assert_eq!(h.binding.admit(&other_table, &h.context(), &h.headers).unwrap_err().code, "FORBIDDEN");
    h.table.replace(h.binding.test_witness_id(), DocumentWitness);
    assert_eq!(h.binding.admit(&h.table, &h.context(), &h.headers).unwrap_err().code, "FORBIDDEN");
}
...
fn D11_Document_RawStartPreservesRequestAndRejectsForgedOwner_09() {
    ...
    let (caller, request) = h.binding.start_request(&h.table, &h.context(), &h.headers, &body).unwrap();
    assert_eq!(caller, h.f.caller);
    ...
    assert_eq!(h.binding.start_request(&h.table, &h.context(), &h.headers, &body).unwrap_err().code, "INVALID_REQUEST");
}
...
fn D11_Document_AuthenticatesBeforeBoundedRawDecode_10() {
    ...
    assert_eq!(h.binding.start_request(&h.table, &h.context(), &HeaderMap::new(), &body).unwrap_err().code, "FORBIDDEN");
    assert_eq!(h.binding.start_request(&h.table, &h.context(), &h.headers, &body).unwrap_err().code, "REQUEST_TOO_LARGE");
    let json = InvokeBody::Json(serde_json::to_value(&h.f.request).unwrap());
    assert_eq!(h.binding.start_request(&h.table, &h.context(), &h.headers, &json).unwrap_err().code, "RAW_BODY_REQUIRED");
    let query = InvokeBody::Raw(vec![b' '; MAX_QUERY_WIRE_BYTES + 1]);
    assert_eq!(h.binding.query_status(&h.table, &h.context(), &h.headers, &query).unwrap_err().code, "REQUEST_TOO_LARGE");
}
...
fn D11_Document_QueryOnlyReturnsOriginalRetainedResult_11() {
    ...
    assert_eq!(h.binding.query_status(&h.table, &h.context(), &h.headers, &unknown).unwrap_err().code, "LAUNCH_NOT_FOUND");
    assert_eq!(h.f.drops.load(Ordering::SeqCst), 0);
}
...
fn D11_Document_RevokeDuringRoutePreventsSpawn_12() {
    ...
    let result = h.f.driver.start_routed(&caller, &h.f.request, || h.f.freeze(), |_| {
        h.authority.revoke();
        Ok(h.f.lease())
    }, |_| panic!("revoked document must not spawn")).unwrap();
...
fn D11_Document_DropRevokesWithoutDiscardingOwnedRun_13() {
    let h = Harness::new(true);
    let status = h.start();
    let resource = h.f.driver.registry().resource(&h.f.caller, &status.run).unwrap();
    drop(h.binding);
    assert_eq!(h.f.driver.registry().status(&h.f.caller, "owned-request").unwrap_err().code, "FORBIDDEN");
    assert_eq!(h.f.drops.load(Ordering::SeqCst), 0);
    h.f.driver.registry().mark_exited(&status.run).unwrap();
...
fn D11_Document_StaleDestroyCannotRevokeReplacement_14() {
    ...
    assert_ne!(h.headers[DOCUMENT_HEADER], next.test_headers()[DOCUMENT_HEADER]);
    assert_ne!(h.f.caller, next.test_caller());
    next.started(&h.url);
    next.finished(&h.url);
    let mut table = ResourceTable::default();
    ...
    assert_eq!(binding.query_status(&table, &h.context(), &next.test_headers(), &query).unwrap_err().code, "LAUNCH_NOT_FOUND");
    assert_eq!(h.query().unwrap_err().code, "FORBIDDEN");
}
...
fn D11_Document_QuerySchemaAndDiagnosticsAreSafe_15() {
    let h = Harness::new(true);
    for bytes in [br#"{"requestId":"owned-request","epoch":"2"}"#.as_slice(), b"not-json-private-value"] {
        let body = InvokeBody::Raw(bytes.to_vec());
        let failure = h.binding.query_status(&h.table, &h.context(), &h.headers, &body).unwrap_err();
