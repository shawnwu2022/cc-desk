...
fn D11_Lifetime_RevokeDropsNativeOwners_001() {
    ...
    assert!(
        ...
        "revoked route retains native manager"
    );
    assert_eq!(route.send(json!(null)).unwrap_err().code, "FORBIDDEN");
    assert_eq!(
        routes
            .bind::<Value>(2, Box::new(|| Ok(())), || panic!("revoked factory"))
            .unwrap_err()
            .code,
        "FORBIDDEN"
    );
...
fn D11_Lifetime_SendFailureDropsGuard_002() {
    ...
    let route = OutputRoutes::new(1)
        .bind(
            ...
            || {
                Ok(Channel::<Value>::new(|_| {
                    Err(std::io::Error::other("fixture").into())
                }))
            },
        )
        .unwrap();
    assert_eq!(
        route.send(json!(null)).unwrap_err().code,
        "OUTPUT_ROUTE_LOST"
    );
    assert!(
        weak.upgrade().is_none(),
        "failed route retains admission guard"
    );
    assert_eq!(
        route.send(json!(null)).unwrap_err().code,
        "OUTPUT_ROUTE_CLOSED"
...
fn D11_Lifetime_RevokeDoesNotWaitForDispatch_003() {
    ...
    assert!(
        weak.upgrade().is_none(),
        "dispatch restored a revoked native owner"
    );
    assert_eq!(route.send(json!(null)).unwrap_err().code, "FORBIDDEN");
}
...
fn D11_Lifetime_ClosedTableRejectsBeforeFactory_005() {
    ...
    assert_eq!(
        routes
            .bind::<Value>(0, Box::new(|| Err(error("FORBIDDEN"))), || panic!(
                "factory"
            ))
            .unwrap_err()
            .code,
