...
use crate::cli::snapshot::{
    availability, discover_candidates, freeze_launch, CallerIdentity, FreezeContext, HostStatus,
};
...
fn caller() -> CallerIdentity {
    CallerIdentity {
        instance_id: "backend-instance".into(),
        window_label: "main".into(),
        webview_epoch: revision("7"),
    }
}
...
fn D08_Snapshot_FreezesProfileEnvironmentAndOwner_01() {
    ...
    let snapshot = freeze_launch(
        &request(&profile, temp.path(), json!({})),
        &profile,
        &owner,
        &FreezeContext {
            inherited: &inherited,
    ...
    assert_eq!(
        snapshot
            .environment()
            .get(OsStr::new("PRIVATE_HOST_VALUE"))
            .unwrap(),
        "original-secret"
    );
...
fn D08_Snapshot_RejectsWrongRevisionAndCliBeforeResolution_02() {
    ...
    let old = request(
        &profile,
        temp.path(),
        json!({"expectedProfileRevision":"0"}),
    );
    assert_eq!(
        freeze_launch(&old, &profile, &caller(), &context)
            .unwrap_err()
            .code,
        "REVISION_CONFLICT"
    );
    ...
    assert_eq!(
        freeze_launch(&wrong, &profile, &caller(), &context)
            .unwrap_err()
            .code,
...
fn D08_Snapshot_RejectsUntrustedCallerAndInvalidCwd_03() {
    ...
    assert_eq!(
        freeze_launch(
            &request(&profile, temp.path(), json!({"launchCwd":"relative"})),
            &profile,
            &caller(),
            &context
        )
...
fn D08_Snapshot_RawPreservesArgumentsAndSuppressesDeskDefaults_05() {
    ...
    let snapshot = freeze_launch(
        &request(
            &profile,
            temp.path(),
            json!({"action":{"kind":"raw","argv":args}}),
        ),
        &profile,
        &caller(),
        &context,
...
fn D08_Snapshot_UnknownExtraArgumentsAreNotFiltered_06() {
    ...
    let context = FreezeContext {
        inherited: &empty,
        terminal: &empty,
        legacy: Some(&json!({"claudeEnvVars":false})),
        observer: None,
    };
    let snapshot = freeze_launch(
        &request(
            &profile,
            temp.path(),
            json!({"extraArgs":["--future-option","opaque secret",""]}),
        ),
        &profile,
        &caller(),
        &context,
...
fn D08_Snapshot_DiscoverySkipsProjectCandidatesAndNeverExecutes_08() {
    ...
    let path =
        std::env::join_paths([project.as_path(), Path::new("."), trusted_dir.as_path()]).unwrap();
    let candidates = discover_candidates(
        "codex",
        &EnvMap::from([("PATH".into(), path)]),
        std::slice::from_ref(&project),
    )
    .unwrap();
    assert_eq!(
        candidates,
        vec![fs::canonicalize(trusted_dir.join(name)).unwrap()]
    );
