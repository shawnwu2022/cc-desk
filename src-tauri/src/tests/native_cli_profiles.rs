...
fn D06_Override_FalseUnsetEmpty_01() {
    assert_eq!(
        resolve_override(Override::Set(false), Some(true)),
        Some(false)
    );
    ...
    assert_eq!(
        resolve_override(Override::Set(String::new()), Some("old".into())),
        Some(String::new())
    );
    assert_eq!(
        resolve_override(
            Override::Set(Vec::<String>::new()),
            Some(vec!["old".into()])
        ),
        Some(vec![])
    );
...
fn D06_Override_WireShape_02() {
    let cases = [
        (json!({"mode":"inherit"}), Override::Inherit),
        (json!({"mode":"set","value":false}), Override::Set(false)),
        (json!({"mode":"unset"}), Override::Unset),
    ];
...
fn D06_Legacy_IsClaudeOnly_03() {
    ...
    assert_eq!(
        Profile::new("codex", CliKind::Codex).resolve_skip_permissions(Some(&legacy)),
        None
    );
    assert_eq!(
        Profile::new("newClaude", CliKind::Claude).resolve_skip_permissions(Some(&legacy)),
        None
    );
...
fn D06_Profile_RejectsWrongLegacyOwnerAndNul_05() {
    assert!(Profile::new("legacyClaude", CliKind::Codex)
        .validate()
        .is_err());
