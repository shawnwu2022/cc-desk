...
fn PtyPaste_NonPasteInputUnchanged_001() {
    for input in ["", "plain 中文🎉", "\x03", "\x16", "\x1bv", "\x1b[A", OPEN, CLOSE] {
...
"#;
    ...
    fn wait_for(output: &Arc<Mutex<String>>, marker: &str) -> Result<(), String> {
        ...
        while start.elapsed() < TIMEOUT {
            if output.lock().unwrap().contains(marker) { return Ok(()); }
    ...
    fn field(output: &str, key: &str) -> Option<u64> {
        output.split_once(key)?.1.chars()
            .take_while(char::is_ascii_digit).collect::<String>().parse().ok()
    }
    ...
    fn assert_native_frames(name: &str, payloads: &[String]) {
        let _guard = NATIVE_LOCK.lock().unwrap_or_else(|error| error.into_inner());
        let pair = native_pty_system().openpty(PtySize {
            rows: 30, cols: 180, pixel_width: 0, pixel_height: 0,
        }).expect("open ConPTY");
        let mut cmd = CommandBuilder::new(node_program());
        cmd.arg("-e");
        cmd.arg(SCRIPT);
        let mut child = pair.slave.spawn_command(cmd).expect("spawn raw stdin probe");
        drop(pair.slave);
        ...
        let result = (|| -> Result<(), String> {
            ...
            writer.write_all(END.as_bytes()).map_err(|e| e.to_string())?;
        ...
        assert_eq!(field(&output, "PASTE_RAW:"), Some(1), "probe must use raw mode");
        assert_eq!(field(&output, "PASTE_BYTES:"), Some(expected.len() as u64), "paste frame byte length mismatch: {name}");
        assert_eq!(field(&output, "PASTE_HASH:"), Some(u64::from(hash(expected.as_bytes()))), "paste frame content mismatch: {name}");
        assert_eq!(field(&output, "PASTE_OPEN:"), Some(1), "paste start delimiter missing");
        assert_eq!(field(&output, "PASTE_CLOSE:"), Some(1), "paste end delimiter missing");
        assert!(output.contains("\x1b[?2004h"), "ConPTY must forward the application's bracketed paste mode to xterm");
        println!("[PASS] {name}: {} UTF-8 bytes including intact paste delimiters", expected.len());
    }

    #[derive(serde::Deserialize)]
    struct Fixture { name: String, prepared: String, repeat: usize }

    #[test]
    fn PtyPaste_NativeFrames_004() {
        let fixtures: Vec<Fixture> = serde_json::from_str(include_str!("../../tests/fixtures/devtools-paste-framing.json")).unwrap();
        for fixture in fixtures {
            let payload = format!("{OPEN}{}{CLOSE}", fixture.prepared.repeat(fixture.repeat));
    ...
    fn PtyPaste_NativeConsecutiveFrames_006() {
        let payloads = [format!("{OPEN}first\n一{CLOSE}"), format!("{OPEN}second\n二{CLOSE}")];
