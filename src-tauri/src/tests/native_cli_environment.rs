...
fn literal(value: &str) -> Override<EnvValue> {
    Override::Set(EnvValue::Literal { value: value.into(), non_secret: true })
}

#[test]
fn D08_Environment_NoCrossProfileLeak_01() {
    let inherited = env(&[("OPENAI_API_KEY", "host-key"), ("ANTHROPIC_API_KEY", "user-key")]);
    let mut claude = Profile::new("claude-one", CliKind::Claude);
    claude.env.insert("ANTHROPIC_BASE_URL".into(), literal("profile-only"));
    ...
    assert_eq!(a.get(OsStr::new("ANTHROPIC_BASE_URL")).unwrap(), "profile-only");
...
fn D08_Environment_EmptyUnsetAndPrecedence_02() {
    ...
    let result = build_environment(&inherited, &env(&[("TERM", "terminal")]), &profile, None, None).unwrap();
...
fn D08_Environment_HostReferenceUsesOriginalHost_03() {
    ...
    profile.env.insert("TARGET".into(), Override::Set(EnvValue::HostRef { name: "SOURCE".into() }));
...
fn D08_Environment_MissingReferenceIsSafe_04() {
    let mut profile = Profile::new("one", CliKind::Codex);
    profile.env.insert("TARGET".into(), Override::Set(EnvValue::HostRef { name: "sensitive-source".into() }));
    let error = build_environment(&EnvMap::new(), &EnvMap::new(), &profile, None, None).unwrap_err();
...
fn D08_Environment_LegacyOnlyAndExplicitOverride_05() {
    ...
    for cli in [CliKind::Codex, CliKind::Claude, CliKind::Shell] {
        ...
        let b = build_environment(&EnvMap::new(), &EnvMap::new(), &new, Some(&json!({"claudeEnvVars": false})), None).unwrap();
        assert!(b.is_empty());
    }
}
...
fn D08_Environment_ObserverIsOptInAndMinimal_06() {
    let observer = ObserverEnv { values: env(&[("CC_BOX_SESSION_ID", "run-only")]) };
    let mut profile = Profile::new("one", CliKind::Claude);
    let off = build_environment(&EnvMap::new(), &EnvMap::new(), &profile, None, Some(&observer)).unwrap();
    assert!(off.is_empty());
    profile.observer = Override::Set(true);
    let on = build_environment(&EnvMap::new(), &EnvMap::new(), &profile, None, Some(&observer)).unwrap();
    assert_eq!(on.get(OsStr::new("CC_BOX_SESSION_ID")).unwrap(), "run-only");
    let unsafe_observer = ObserverEnv { values: env(&[("OPENAI_API_KEY", "observer-secret")]) };
    let error = build_environment(&EnvMap::new(), &EnvMap::new(), &profile, None, Some(&unsafe_observer)).unwrap_err();
...
fn D08_Environment_RejectsUnsafeValuesWithoutMutation_07() {
    ...
    assert!(build_environment(&env(&[("BAD=NAME", "secret")]), &EnvMap::new(), &Profile::new("one", CliKind::Codex), None, None).is_err());
}

#[test]
fn D08_Environment_TerminalCannotOverrideProvider_08() {
    let error = build_environment(&EnvMap::new(), &env(&[("OPENAI_API_KEY", "secret")]), &Profile::new("one", CliKind::Codex), None, None).unwrap_err();
    assert_eq!(error.code, "INVALID_REQUEST");
...
fn D08_Environment_WindowsCaseConflictAndDeletion_09() {
    ...
    assert!(build_environment(&env(&[("PATH", "first"), ("Path", "second")]), &EnvMap::new(), &profile, None, None).is_err());
    let mut profile = profile;
    profile.env.insert("PATH".into(), Override::Unset);
    let result = build_environment(&env(&[("Path", "old")]), &EnvMap::new(), &profile, None, None).unwrap();
...
fn D08_Environment_WindowsHostRefAndProfileAliasConflict_10() {
    let mut profile = Profile::new("one", CliKind::Codex);
    profile.env.insert("TARGET".into(), Override::Set(EnvValue::HostRef { name: "path".into() }));
    let result = build_environment(&env(&[("Path", "host")]), &EnvMap::new(), &profile, None, None).unwrap();
...
fn D08_Environment_UnixPreservesCaseAndNonUnicodeValues_11() {
    ...
    let result = build_environment(&inherited, &EnvMap::new(), &Profile::new("one", CliKind::Codex), None, None).unwrap();
